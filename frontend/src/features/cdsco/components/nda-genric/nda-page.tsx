...
export function NdaPage() {
    ...
    const [result, setResult] = useState<{ "drug name": string; "Eligibility": string } | null>(null);
    ...
    const handleCheck = async () => {
        ...
        try {
            const data = await api.common.analyzeDossier(formData);
            ...
            <header className="bg-[#1E3A5F] shadow-md">
                <div className="max-w-6xl mx-auto px-6 py-5">
                    ...
                    <p className="text-blue-200 text-sm mt-1">
                        Upload and manage NDA submission files
                    </p>
                </div>
            </header>
                        ...
                        <div className={`border-2 border-dashed rounded-2xl transition cursor-pointer max-w-3xl h-64 mx-auto ${file ? 'border-green-300 bg-green-50' : 'border-blue-200 bg-blue-50 hover:bg-blue-100'}`}>
                            ...
                            >
                                ...
                                <p className="text-lg font-medium text-gray-700 mb-2">
                                    {file ? file.name : "Drag & Drop NDA File Here"}
                                </p>
                            ...
                            <div className="mt-8 max-w-3xl mx-auto p-6 bg-white border border-gray-200 rounded-2xl shadow-sm">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="pt-2 border-t border-gray-100">
                                        <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Eligibility</label>
                                        <p className={`text-base font-medium mt-1 p-3 rounded-lg ${result.Eligibility.includes("Generic") ? 'bg-orange-50 text-orange-800 border border-orange-100' : 'bg-green-50 text-green-800 border border-green-100'}`}>
                                            {result.Eligibility}
                                        </p>
                                    </div>
                                </div>
